...
const DashMyPosts = () => {
	...
	useEffect(() => {
		...
		const fetchPosts = async () => {
			try {
				const res = await fetch(`/api/post/getposts?userId=${currentUser._id}&limit=10`);
									...
									<Table.Row key={post._id} className="border border-gray-400">
										...
										<Table.Cell>
											<Link to={`/update-post/${post._id}`}>
												<button className="text-teal-400 dark:text-gray-400 hover:text-teal-600 dark:hover:text-gray-200">
													Edit
												</button>
											</Link>
										</Table.Cell>
